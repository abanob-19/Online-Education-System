...
const viewAllCoursesWithPrice = asyncHandler(async (req, res) => {
  const table = await Course.find(
    {},
    {  }
  );
...
const getInstructorRatings = asyncHandler(async (req, res) => {
  ...
  const user = await users.findById(instructorId, { rating: 1 });
...
//   const refundedCourse = await refund.findOne({ CourseID: CourseId });

//   if (refundedCourse.percent < 50) {
//     refundedCourse.amount += course.price;
//   } else {
//     throw new Error("You already attended more than 50% of this course");
//   }
// });
