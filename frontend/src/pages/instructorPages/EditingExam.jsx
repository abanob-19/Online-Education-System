...
import { editExam, reset } from "../../features/instructor/instructorSlice";

function EditingExam() {
  ...
  const [questionList, setQusetionList] = useState(currExam.questions);

  const [formData, setFormData] = useState({
    examTitle: currExam.title,
    text: "",
    choiceA: "",
    choiceB: "",
    choiceC: "",
  ...
  function editExamClick() {
    if (examTitle === "") {
      ...
      if (questionList.length === 0) {
        ...
        const exam = {
          examNumber: currExam.number,
          courseID: currCourse._id,
          title: examTitle,
          updatedQuestionList: questionList,
        };
        toast.success("Exam Edited successfuly");
    ...
    <div class='box-form' style={{width:'1500px'}}>
      ...
      {(formVisible || editing) && (
        <section className="form">
          ...
          <div className="form-group">
            <button onClick={onClickCancel} className="btn btn-block">
              Cancel
            </button>
          </div>
        </section>
      ...
      {!formVisible && (
        <><button onClick={onClickAddQuestion}> Add a new Question </button>
      

      <button className="btn " onClick={editExamClick}>
        {" "}
        Edit Exam{" "}
      </button>
      </>)}
